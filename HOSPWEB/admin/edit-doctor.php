...
if(isset($_POST['submit']))
{
	...
	if(mysqli_num_rows($result)==0)
	{
		...
		echo"<script>window.open('edit-doctor.php','_self')</script>";
	}

$quer = "SELECT *  FROM `specialization` where special='$special'";
   $result1 = mysqli_query($con, $quer);
   $row1 = mysqli_fetch_array($result1);
   $s=$row1['sno'];

//    $qe="UPDATE `doctor` SET 
//    `fullname`='$name',
//    `fees`='$fees',
//    `city`='$city',
//    `address`='$address',
...
//     WHERE email=$add";
    ...
    // $sql = "UPDATE `doctor` SET `fullname`=`$name`,`fees`=`$fees`,`city`=`$city`,`address`=`$address`,`contact`=`$contact`,`sno`=`$s`,`password`=`$password` WHERE `password`=`imran`";
    $sql = "UPDATE `doctor` SET fullname='$name',fees=$fees,city='$city',address='$address',contact='$contact',sno=$s,password='$password' WHERE email='$email'";
 ...
 <nav class="navbar navbar-expand-lg bg-light pl-8">
		...
		<div class="row" style="margin-left: 200px;">
			<div class="main-login col-xs-10 col-xs-offset-1 col-sm-8 col-sm-offset-2 col-md-4 col-md-offset-4">
				...
				<a href="doctor-panel.html"><h2>Edit Doctor profile .</h2></a>
				</div>
				<!-- start: REGISTER BOX -->
